  ...
  try {
    ...
    admin
      ...
      .then(async () => {
        ...
        await updateDoc(userRef, { notifications: arrayUnion(notificationId) });
    ...
    admin
      ...
      .then(async () => {
        ...
        for (const nid of userData.notifications) {
          const notificationSnapshot = await getDoc(doc(db, "notifications", nid));
          const notificationData = notificationSnapshot.data();
          if (notificationData.postId === postId && notificationData.sender === username) {
            notificationId = nid;
            break;
          }
        }
        ...
        if (notificationId) {
          ...
          const notificationIndex = userData.notifications.indexOf(notificationId);
          userData.notifications.splice(notificationIndex, 1);

          // Update the notifications array in the database
          await updateDoc(userRef, { notifications: userData.notifications });
        }
  ...
  try {
    ...
    admin
      ...
      .then(async () => {
        ...
        await updateDoc(storyRef, { likes: arrayUnion(username) });
  ...
  try {
    ...
    admin
      ...
      .then(async () => {
        ...
        await updateDoc(storyRef, { likes: arrayRemove(username) });
        ...
        for (const nid of userData.notifications) {
          const notificationSnapshot = await getDoc(doc(db, "notifications", nid));
          const notificationData = notificationSnapshot.data();
          if (notificationData.storyId === storyId && notificationData.sender === username) {
            notificationId = nid;
            break;
          }
        }
  ...
  try {
    ...
    admin
      ...
      .then(async () => {
        ...
        await setDoc(doc(db, 'notifications', nid), notificationData);
  ...
  try {
    ...
    admin
      ...
      .then(async () => {
        ...
        const userRef = doc(db, 'users', (await getDoc(postRef)).data().createdBy);
        const userSnapshot = await getDoc(userRef);
        const userData = userSnapshot.data();
        ...
        for (const nid of userData.notifications) {
          const notificationSnapshot = await getDoc(doc(db, 'notifications', nid));
          const notificationData = notificationSnapshot.data();
          console.log("notificationData.commentId: ", notificationData.commentId);
          console.log("commentId: ", commentId);
          if (notificationData.commentId === commentId && notificationData.sender === username) {
            notificationId = nid;
            break;
          }
        }
